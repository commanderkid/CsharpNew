...
namespace TextAnalysis
{
    static class SentencesParserTask
    {
        ...
        {
            //text = "abc! abc huy.";
            List<char> temporaryListForChar = new List<char>(); //array for char
            List<string> temporaryListForString = new List<string>(); //char convert to word
            text.ToLower(); //convert all text to lower
            var sentencesList = new List<List<string>>(); // return list of statements of words
            foreach (char symbol in text) //iteration for each sembol in text
            {
                if (char.IsLower(symbol) || symbol == '\'')
                    temporaryListForChar.Add(symbol);
                else if ((symbol == ' ' || symbol == ',') && temporaryListForChar.Count != 0)
                {
                    temporaryListForString.Add(new string(temporaryListForChar.ToArray()));
                    temporaryListForChar.Clear();
                }
                else if ((symbol == '.' || symbol == '!' || symbol == '?' || symbol == ';' ||
                         symbol == ':' || symbol == '(' || symbol == ')') && temporaryListForChar.Count != 0)
                {
                    temporaryListForString.Add(new string(temporaryListForChar.ToArray()));
                    temporaryListForChar.Clear();
                    sentencesList.Add(temporaryListForString);
                    temporaryListForString.Clear();
                }
